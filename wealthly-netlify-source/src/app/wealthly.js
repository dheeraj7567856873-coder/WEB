...
const ICONS = {
  ...
  'right-from-bracket': '<path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/>',
  'camera': '<path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/>',
  'lock': '<rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>',
  'file-import': '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><polyline points="9 15 12 12 15 15"/><line x1="12" y1="12" x2="12" y2="19"/>',
  'floppy-disk': '<path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/>',
};
...
/* ---------------------- 2. API PERSISTENCE HELPERS ---------------------- */
/* LocalStorage has been replaced by REST API calls. Nothing is stored on the
   device: every read and write is scoped to the signed-in user account. */

let profile = { id: '', full_name: '', email: '', avatar_url: '' };

function saveState() {
  // Settings are the only slice persisted implicitly; records persist on write.
  void api.saveSettings(state.settings).catch(apiError);
}
...
// jsPDF's built-in fonts are WinAnsi-only: symbols like Rs, EUR, GBP, JPY
// (and especially the rupee sign) render as blank or garbled glyphs, which
// makes the exported report look broken. Use ASCII currency codes instead.
function pdfMoney(amount) {
  const code = (state.settings && state.settings.currency) || 'INR';
  const rounded = Math.round((Number(amount) + Number.EPSILON) * 100) / 100;
  return code + ' ' + rounded.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}
      ...
      <tr>
        ...
        <td>
          <div class="row-actions">
            <button class="icon-btn" title="View" data-row-action="view" data-id="${t.id}" data-type="${t.type}"><i class="fa-solid fa-eye"></i></button>
            <button class="icon-btn" title="Edit" data-row-action="edit" data-id="${t.id}" data-type="${t.type}"><i class="fa-solid fa-pen"></i></button>
            <button class="icon-btn" title="Delete" data-row-action="delete" data-id="${t.id}" data-type="${t.type}"><i class="fa-solid fa-trash"></i></button>
          </div>
        </td>
...
function safe(label, fn) {
  try {
    ...
    console.error(`[wealthly] ${label} failed`, err);
    return undefined;
  }
}

export async function initWealthly() {
  // The loading screen must never be able to trap the user: whatever happens
  // below, it is removed in the finally block.
  try {
    safe('icons', replaceAllIcons);
    safe('iconObserver', initIconObserver);

    try {
      // Never let a slow/hanging network request keep the app on the splash.
      await Promise.race([
        loadState(),
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Loading your data timed out. Pull to refresh or try again.')), 15000),
        ),
      ]);
    } catch (err) {
      apiError(err);
    ...
    safe('applyProfileToUI', applyProfileToUI);
    safe('initNavigation', initNavigation);
    safe('initTopbarActions', initTopbarActions);
    safe('initForms', initForms);
    safe('initFilters', initFilters);
    safe('initRowActions', initRowActions);

    safe('initModals', initModals);
    safe('initFab', initFab);
    safe('initCalendarNav', initCalendarNav);
    safe('initExportImport', initExportImport);
    safe('initProfilePage', initProfilePage);
    ...
    safe('dateInputs', () => {
      ...
      if (budgetMonth) budgetMonth.value = currentMonthKey();
    });
  } finally {
...
const PAGE_TITLES = {
  ...
  profile: ['Profile', 'Manage your account, password and data'],
};
...
function initForms() {
  document.getElementById('expenseForm').addEventListener('submit', async (e) => {
    ...
    let created;
    try {
      created = await api.createExpense({
        date,
        amount,
  ...
  document.getElementById('incomeForm').addEventListener('submit', async (e) => {
    ...
    let created;
    try {
      created = await api.createIncome({
        date,
        amount,
  ...
  document.getElementById('budgetForm').addEventListener('submit', async (e) => {
    ...
    try { await api.setBudget(month, amount); } catch (err) { apiError(err); return; }
  ...
  document.getElementById('editForm').addEventListener('submit', async (e) => {
    ...
    try {
      if (type === 'expense') await api.updateExpense(record);
      else await api.updateIncome(record);
    } catch (err) { apiError(err); return; }
...
function initRowActions() {
  ...
  tbody.addEventListener('click', (e) => {
    ...
    const { rowAction, id, type } = btn.dataset;
    if (rowAction === 'view') viewTransaction(id, type);
    else if (rowAction === 'edit') openEditModal(id, type);
    else if (rowAction === 'delete') requestDelete(id, type);
  });
}
...
function initExportImport() {
  const monthField = document.getElementById('monthlyReportMonth');
  if (monthField && !monthField.value) monthField.value = currentMonthKey();
  ['monthlyPdfBtn', 'exportPdfBtn', 'exportPdfReportBtn'].forEach((id) => {
    const btn = document.getElementById(id);
    if (btn) btn.addEventListener('click', exportMonthlyPdf);
  });

  document.getElementById('exportExcelBtn').addEventListener('click', exportExcel);
  const input = document.getElementById('importExcelInput');
  document.getElementById('importExcelBtn').addEventListener('click', () => input.click());
  ...
  try {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(
      state.expenses.map(e => ({
        Date: e.date, Amount: Number(e.amount), Category: e.category,
        Payment: e.paymentMethod || '', Description: e.description || '',
      }))), 'Expenses');
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(
      state.incomes.map(i => ({
        Date: i.date, Amount: Number(i.amount), Source: i.source, Description: i.description || '',
      }))), 'Incomes');
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(
      Object.keys(state.budgets).sort().map(m => ({ Month: m, Amount: Number(state.budgets[m]) }))), 'Budgets');
    XLSX.writeFile(wb, `wealthly-data-${localToday()}.xlsx`);
...
async function importExcel(event) {
  ...
  try {
    ...
    const expenses = sheet('Expenses').map(r => ({
      date: normaliseSheetDate(r.Date ?? r.date),
      amount: Number(r.Amount ?? r.amount),
      category: String(r.Category ?? r.category ?? 'Other'),
      paymentMethod: String(r.Payment ?? r.PaymentMethod ?? r.payment ?? 'Cash'),
      description: String(r.Description ?? r.description ?? ''),
    })).filter(r => r.date && r.amount > 0 && !isFutureDate(r.date));

    const incomes = sheet('Incomes').map(r => ({
      date: normaliseSheetDate(r.Date ?? r.date),
      amount: Number(r.Amount ?? r.amount),
      source: String(r.Source ?? r.source ?? 'Other'),
      description: String(r.Description ?? r.description ?? ''),
    })).filter(r => r.date && r.amount > 0 && !isFutureDate(r.date));

    const budgets = sheet('Budgets').map(r => ({
      month: String(r.Month ?? r.month ?? '').slice(0, 7),
      amount: Number(r.Amount ?? r.amount),
    })).filter(r => /^\d{4}-\d{2}$/.test(r.month) && r.amount > 0);

    if (!expenses.length && !incomes.length && !budgets.length) {
      showToast('No valid rows found. Expected Expenses / Incomes / Budgets sheets.', 'error');
      return;
    }

    await api.bulkImport({ expenses, incomes, budgets });
    await loadState();
    applySettingsToUI();
    renderAll();
    showToast(`Imported ${expenses.length} expenses, ${incomes.length} incomes, ${budgets.length} budgets`, 'success');
  } catch (err) {
    apiError(err);
  }
}

/* ---------------------- 11. PROFILE, PASSWORD, AVATAR, LOGOUT ---------------------- */
function applyProfileToUI() {
  const name = profile.full_name || (profile.email || '').split('@')[0] || 'Your account';
  ...
  if (profile.avatar_url) {
    ...
    initials.textContent = name.trim().charAt(0).toUpperCase() || 'W';
  }
}
...
function initProfilePage() {
  document.getElementById('profileForm').addEventListener('submit', async (e) => {
    ...
    if (fullName.length > 80) { showToast('Name must be 80 characters or fewer', 'error'); return; }
    try {
  ...
  document.getElementById('avatarPickBtn').addEventListener('click', () => avatarInput.click());
  avatarInput.addEventListener('change', async (e) => {
    const file = e.target.files && e.target.files[0];
    e.target.value = '';
    if (!file) return;
    if (!file.type.startsWith('image/')) { showToast('Please choose an image file', 'error'); return; }
    if (file.size > 3 * 1024 * 1024) { showToast('Image must be smaller than 3 MB', 'error'); return; }
    try {
  ...
  document.getElementById('logoutBtn').addEventListener('click', async () => {
    try { await api.logout(); } catch (err) { apiError(err); }
  });
}
...
function exportMonthlyPdf() {
  try {
    ...
    const month = (monthInput && monthInput.value) || currentMonthKey();
    const monthTransactions = getAllTransactions()
      .filter(t => t.date && monthKey(t.date) === month)
      .sort((a, b) => new Date(a.date) - new Date(b.date));

    if (monthTransactions.length === 0) {
      showToast('No transactions found for ' + monthLabel(month) + '. Pick another month.', 'error');
      return;
    }
    ...
    const boxY = 34, boxW = 58, boxH = 22, gap = 4;
    const summary = [
      { label: 'Total Income', value: pdfMoney(income), color: [16, 185, 129] },
      { label: 'Total Expense', value: pdfMoney(expense), color: [251, 113, 133] },
      { label: 'Net Balance', value: pdfMoney(balance), color: balance >= 0 ? [56, 189, 248] : [239, 68, 68] },
    ];
    ...
    const rows = monthTransactions.map(t => [
      ...
      String((t.type === 'expense' ? t.category : t.source) || '-'),
      String(t.description || '-'),
      (t.type === 'expense' ? '-' : '+') + pdfMoney(t.amount),
    ]);
    ...
    showToast('Could not generate the monthly PDF: ' + ((err && err.message) || 'unknown error'), 'error');
  }
}
